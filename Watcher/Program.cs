...
class DirectorySync
{
    ...
    private static void LogChange(string fileName, string action)
    {
        ...
        {
            connection.Open();
            string sql = "INSERT INTO FileChanges (FileName, Action, Timestamp) VALUES (@FileName, @Action, @Timestamp)";
            connection.Execute(sql, new { FileName = fileName, Action = action, Timestamp = DateTime.Now });
        }
    }
    ...
    private static void EnsureDatabaseExists()
    {
        ...
        {
            connection.Open();
            string sql = "CREATE TABLE IF NOT EXISTS FileChanges (Id INTEGER PRIMARY KEY, FileName TEXT, Action TEXT, Timestamp DATETIME)";
